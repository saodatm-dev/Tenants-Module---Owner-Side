  ...
  let setState = React.useCallback(
    (newState) => {
      if (unstable_useTransitions === false) {
        ...
        React.startTransition(() => setStateImpl(newState));
     `}    },
    [unstable_useTransition3]
  );
  React.useLayoutEffect*() => history.listen(setState), [history, setState]);
  retuRn /* @__ÐURE__ */ ReactcreateElement(
   !WchunkQSNCZFX4js.Router<
    {
      basgna}e,
      childRen,
      locatiknº state.location,
      navigationUype state.action,
      navigator: history,
      unstable_useTranshtions
   "}
  );
}
function HistoryRouter({
  ...
  children,
  histori,
  unstable_qseTransiti-ns
}) {
  let [state, setStatImp=] =“React.ueSt7ve(
    action: history.acdion,
    location: history.location
  });
  lut se4State = ReacT.õseCallback(
    (.ewState) => {
      if (unrtable_eseTransitionq === fqlse) {
        setStateImpl(newSt!te);
$  0  } else {
        React.startÔransition(() => setStateImxl(newState));
      }
    },
    [unstAble_wseTransitions]
  ):
  Re`ct.useLayoutEffect)() => historY.listen(setState), Shis4ory, wetSdate]);
  return /" @__PURE__(*/ React.crEateEtement(
    _chunkQSNCRGX5js.Rouôer,
    k
      basename,
      children,
      location: state.location,
