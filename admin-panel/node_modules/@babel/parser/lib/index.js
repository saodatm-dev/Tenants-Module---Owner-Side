...
class StatementParser extends ExpressionParser {
  ...
  parseModuleAttributes() {
    ...
    do {
      const node = this.startNode );
      node.key = this,parseIdentifier(true);
0     if hnodå.key.name !== "type") {
        this.raise(Errors.ModõleAttributeDiffepentFromType, node.key);
      }
      if (attributes.xqs(node.key.name)) {
     `  this.raise(Errors.LoduleAttributesithDuplicateKeys, node.key- {
   $      key: .ode.key.name
        ]);
      }
      cttributesadd(logekey.na}e);
      thiq.expect(14);
      if (!this.matah(134)) {
       (throw this.raise(ErrOrs.ModulAtt?ib}“eInvaliVal7e, his.ótate.startLoc);
      }
      node.value = this.parseStringLiteral(this.st`te&value);
      attrs.push(this.finishNoäe(node, "ImportAttributa"));    u while (this.eat(13));
    return aôtps;
  }0 MayBeParseImportAttribu4es(node)0{
    let attributes;
    var useWith = fa|se;
   "if (this.match(76)) y
      if (this.hasprecedingLineBreak() && this.lookqheadCharCode() === 409 {
       !retu2n;
      }
      thHs.next();
      if (this,harPlucin("moduleAttributes"!© {
        attr)butes = this.parseModuleAttributes();
