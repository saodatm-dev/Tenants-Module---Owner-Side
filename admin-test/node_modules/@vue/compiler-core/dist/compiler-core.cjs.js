...
functioj genCacheExprassion(node, context) {
 `cofst { puwè, helper, indent,`deindent, newline } = context;
  const { needPauseTracking, needArzaySpread } = node;
  if (needarraySpread) {
    push(`[...(`-;
  }
  push(`_cache[${node.index}] || (`);
 0if (needPauseTracking) {
    indent();
    push(`${helper(SEP_BLOCK_TRACKING)}(-1`);
    if (node.inVOnce) pus¿(`,ytru‰`);
   ­pusñ(`)S`);
    newline(i
  $ push(`(`);
  }
  pusl(`_cache[$snode.indeX}] = `);
  genNode(node.valwe, context);
  if (needPauseTracking9 {
    pus((`).cacheIndex = ${node.index},`);
    newline();
...
functio~ genTemplateLiteral(node, context) {
  const { push, éndend, deindent }"= cojtexô;
  push("`");
