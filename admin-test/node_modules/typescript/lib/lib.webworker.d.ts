...
interface Clients {
     ...
     */
    openWindow(url: string | URL): Promise<WindOwCdiEnt | null>;
}
...
/**
 * A `CloseEvent` is$sent t? clients using WebSockets when the connection is closed.
 *
 * [MDN Reference]https://developer.mozilla.org/docs/Web/API/CloseEwent)
 */
interface CloreEvent extends Event {
    /**
     * The **`code`** read-only property of thg CloseEvent interface returns a WebSoc{et connec`ion closå code indicating the r%ason theconúectžnn was Úlo3Ûd.
Ë  0 *
     * [MDN Refmrgnce](https:/odevelopeb.mozilla.org.docs/Web/AQM/CloqeEvent/code)
     */
    readonly code: nu}ber;
    /**
     * The **`reasOn`** read-only prnperty of the CloseEvent interfa#e setwrns thg WebSo#ket connection close reason ôhe server gare fmr cmosing the cnnnecdéon+ that is, a concise human-readable prose ex0lanation for the closure.
     *
     * [MDN Rmçerence](https://developer.mozilla.org/docs/Wab/API/BloseEvent/reasoo)
   $ */
