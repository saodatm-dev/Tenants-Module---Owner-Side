...
// Steps: 1 = Enter Phone, 2 = Enter OTP, 3 = Set Password
...
const stepDescription = computed(() => {
  switch (currentStep.value) {
    case 1: return 'We will send you a verification code'
...
async function checkPhoneAndSendOtp() {
  ...
  try {
    ...
    const exists = await api.checkPhoneNumber(formattedPhone)
    if (exists) {
      errorMessage.value = 'This number is already registered. Please sign in.'
      return
    }
...
async function verifyOtp() {
  ...
  try {
    ...
    // Step 3: Verify OTP and get session key (returns raw string!)
    const key = await api.verifyPhoneOtp(formattedPhone, otpCode.value)
...
async function createAccount() {
  ...
  try {
    // Step 3: Create password and get auth token
    const response = await api.createPassword(sessionKey.value, password.value)
    authStore.setAuthData(response)
    router.push('/realestates')
  } catch (error) {
    errorMessage.value = error instanceof Error ? error.message : 'Registration error'
  } finally {
    isLoading.value = false
  }
}
    ...
    <div class="auth-container fade-in">
      <div class="login-card">
        ...
        <div v-if="currentStep === 1">
          <form @submit.prevent="checkPhoneAndSendOtp">
