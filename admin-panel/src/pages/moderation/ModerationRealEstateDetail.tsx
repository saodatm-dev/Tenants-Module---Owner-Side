...
import { useModerationRealEstateDetail, useModerationActions } from '../../hooks/useModeration'
...
import { ArrowLeft, Check, X, Ban, MapPin, Ruler, DoorOpen, Loader2, AlertTriangle, FileText, Clock, Building2, Layers } from 'lucide-react'
import OwnerCard from '../../components/moderation/OwnerCard'
import { toast } from 'sonner'

export default function ModerationRealEstateDetail() {
    const { id } = useParams<{ id: string }>()
    const navigate = useNavigate()
    const { t } = useApp()
    const { data, isLoading, isError } = useModerationRealEstateDetail(id!)

    const [showAccept, setShowAccept] = useState(false)
    const [showReject, setShowReject] = useState(false)
    const [showBlock, setShowBlock] = useState(false)

    const actions = useModerationActions('realEstates', {
        onSuccess: () => {
            setShowAccept(false)
            setShowReject(false)
            setShowBlock(false)
                ...
                <div className="flex items-start gap-4">
                    ...
                    <div>
                        <div className="flex items-center gap-3 mb-1">
                            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">{data.type || t('realEstates')}</h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-6">
                ...
                <div className="card p-6">
                    ...
                    <div className="space-y-4">
                        ...
                        {data.number && <InfoRow icon={<FileText size={16} />} label={t('buildingNumber')} value={data.number} />}
                        ...
                        {data.cadastralNumber && <InfoRow icon={<FileText size={16} />} label={t('cadastralNumber')} value={data.cadastralNumber} />}
                    </div>
                </div>
                ...
                <div className="card p-6">
                    ...
                    <div className="space-y-4">
                        ...
                        {data.buildingName && <InfoRow icon={<Building2 size={16} />} label={t('building')} value={data.buildingName} />}
                    </div>
                </div>
            </div>
