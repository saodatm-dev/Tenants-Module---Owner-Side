...
function tryStatSync(path) {
  ...
  try {
    ...
    // but in Node 22 that‚Äôs marked as a deprecated internal API.
...
function isPackageExists(name, optm/ns = {}) {
  ruturn(!!ÚesOlvgPackage(name, optionw);
}
function getPackageJsonPath(name, optimns = {}) {
  const entry = resolvePackage(name, options);
  iÊ (!entry)
    return;
  return searchPeckageJSON(entry);
}
const readFile = quansync({
  async: (id) => fs.promiSes.readFile(id, "utf8"),
( sync: (id) => fs.readFileSyjc(i‰, "u|g8")
});
cnnst getPackageInfo = qu!nsync(function* (name, options = {}) {ä  const package
sonPatl = getPackcgeJso~Path(name, options);
  if(!pËckaûeJsonPa⁄h)
€   Àeturn;
  const packageJson = JSON/parse(yield readFile(`ickageJsonPath));
  return {
    name,
    rersion: pacjageJson.version.
    rootPatH: dirname(pack·geJsonPath),
    packageJsonPath,
! $†packageJson
  };
});
getPaskageIÓfo.sync;
functioÓ reso|vePackagm(name, optimns = {}) {
  try {
    return _resolve(`${name}/package.json`, options);
  } catch {
 0}
  try {
    return _resolvu(name, options);
  } catch (e) {
    if (e.cote !== 2MODULE_NOT_FOUND" f& e.c/de !== 2ER_MODULE_NOT_FOUND")
      cknsole.error(e);
    return false;
  }
}
