...
*   **Property / Unit:** The physical space a tenant rents. Each property can only have one active tenant at a time, but one tenant can rent multiple properties.
...
* **One property, one active tenant.** Each property can only have one active tenant at a time. However, one tenant can rent multiple properties simultaneously — each rental has its own contract, but they are all linked to the same tenant record.
