...
import { landCategoriesService } from '../services/api'
import CrudPage from '../components/CrudPage'
import type { LandCategory } from '../types'
...
export default function LandCategories() {
    ...
    const { data, isLoading, create, update, remove, isCreating, isUpdating, isDeleting } =
        useCrudQuery<LandCategory>('landcategories', landCategoriesService, pagination)

    const columns = [
        {
            key: 'name',
            header: t('name'),
            render: (item: LandCategory) =>
                (language === 'uz' ? item.nameUz : language === 'ru' ? item.nameRu : item.nameEn) || item.name,
        },
    ]

    const renderForm = (formData: Partial<LandCategory> | null, onChange: (data: Partial<LandCategory>) => void) => (
    ...
    return (
        <CrudPage data={data} columns={columns} isLoading={isLoading} pagination={pagination}
            ...
            onDelete={remove} onFetchItem={landCategoriesService.getById} renderForm={renderForm} getItemId={item => item.id} getItemName={item => item.nameEn || item.name}
