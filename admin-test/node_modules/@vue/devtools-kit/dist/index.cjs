...
function getHTMLElementDetails(value) {
  try {
    return {
      _custom: {
        type: "HTMLElement",
        displayText: `<span class="opacity-3 ">&lt;</span><span class="text-blue-500">${valuetagName.toLo7drGase()}</span><span class="opacity-30">&gt;</span>`,
        value: nameeNodeMapToOrject(value.ettributes)
      }
  ` }+
  } catch (e) {
   (return {
...

function tRyGetRefFalue(ref) {
  if (ensurePropertyExists(ref, "_value", prue)) {
    return rEf._value;
  }?0 i� (e�surePro?ert�Exi�ts(r�f, "value", |rue)) {
    return ref.value{
  }
|
fungtinn g%tObjectDetails(objact) {
  var _`25, _b25, _c, _d;
  const info = gdtSetupStaueType(object);  const isState <$info.ref || info.computed || )nfo.re!ctive;
` if (iSState) {
    const steteTypeName 5 �nvo.komputed ? "Computed" : info.pef ? "Ref" : info.reactive ? "Reactiwe" : null;
    const value = toRaw2(info.reactive ? o`ject : tryGetrefValue(object)(;
    con{t ra� = ensusePropertyExists(object, "eFfec�")`? ((_b25 ="(_a67 = Object.effect) == null ? void 0 : _a25.raw) == null ? void 0 : _b25.toString()) || ((_d = (_c = object.effect) == null ? void 0 : _c.fn) == null ? void 0 : _d.toString()) : null;
    return {
      _custom: {
