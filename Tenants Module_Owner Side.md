...
| # | Tab | What It Shows |
|---|-----|---------------|
| 1 | **Overview** | Summary card: tenant name, phone, email, current status badge, linked property/unit, contract dates, and current balance (owed or overpaid). Also includes a timeline of all recorded activity for this tenant (status changes, contract renewals, payment events, notes added, documents uploaded). This is the default tab when the cabinet opens. |
...
| 3 | **Payments** | Payment history table: date, amount, method, and status (paid, overdue, partially paid). Shows current balance at the top. Includes a link to the Payments module for detailed financial records. |
| 4 | **Documents** | All documents tied to this tenant: signed contracts, invoices, and any files uploaded by the owner. Contract details (start and end dates, rent amount, deposit, payment schedule, renewal history) are accessible here. Documents can be uploaded, downloaded, and previewed. |
| 5 | **Notes** | Free-form notes the owner or property manager can add about the tenant. Every note records the date and the person who wrote it. |
...
* The **activity timeline** in the Overview tab is read-only — it is automatically generated by the system and cannot be edited.
* If a tenant has **multiple active rentals** (multiple units), the Property/Unit and Documents tabs show each rental separately, clearly labeled.
