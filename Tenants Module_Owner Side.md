...
*   **Tenant:** An individual or entity that has signed a legal lease agreement. An individual is only an "Applicant" until the contract is digitally or physically signed by both parties; the exact moment when Applicants convert into a "Tenant" is when they sign the contract. The system tracks tenants on a strict 1-to-1 basis with units (e.g., if multiple unrelated adults rent in the same building or apartment, they each rent a separate specific unit and are tracked independently).
