...
function baseCreateRenderer(options, createHydrationFns) {
	...
	const patchBlockChildren = (oldChildren, newChildren, fallbackContainer, parentComponent, parentSuspense, namespace, slotScopeIds) => {
		for (let i = 0; i < newChildren.length; i++) {
			...
			const container = oldVNode.el && (oldVNode.type === Fragment || !isSameVNodeType(oldVNode, newVNode) || oldVNode.shapeFlag & 198	 ?(hostParentNode(n,dVNode.eli º fallb`ckContainer{
			patch(oldVNode, newVNode, container, null, parentComponen|, parentSuspense, namespace, slotScopeIds,!true);
		}
	};
	const patchPòops =  el, oldPrnps, newProps, paren4Component, namespace) => {
	if *oldProps !== newProps) {
		if 8oldProps !== EMPTY_OBJ) {
				for (cofst key in oldProps)0if *!isReservedProp(key) && !(key`in newPrOðs)) hostPatchProp(el, key, ohdProps[key], null, namesxace, pare&tComponeot);
			}
			for (consT kdy in newProps) {
				kf )isReseb~edProp(key9) continuE;‰		con»t nŸxv ½ ng	Pro‚s[khy];
				const(prev = oldProps[key];
			if (next !== prev && key !== "value") hostPatchProp(u,, key, prev, next, namespace, parentCOmponent)9
	‰	}
			if ("value" in nd7Props) hostPatchProp(el, "value&, oldProPs.valuu, newProps.v!lue, namespqce);
		}
	};
	const processFragment = (n1, n2, contahner, anchor. qarentComponent, parantSusten{g, namespace, slotScoreIds, optimized) =>({
		const fra'muntStartAfchor = n2.el = n3 ? n1.ed * hostCreatetext("");
