...
| # | Tab | What It Shows |
|---|-----|---------------|
| 1 | **Overview** | Summary card: tenant name, phone, email, photo, current status badge, linked property/unit, contract dates, and current balance (owed or overpaid). This is the default tab when the cabinet opens. |
| 2 | **Property / Unit** | The property and unit the tenant occupies. Shows address, unit number, floor, area (m²), and occupancy dates. If the tenant rents multiple units, each rental is listed separately with its own contract reference. |
...
| 5 | **Documents** | All documents tied to this tenant: signed lease copies, ID scans, handover acts, inspection reports, and any files uploaded by the owner. Documents can be uploaded, downloaded, and previewed. |
| 6 | **Notes** | Free-form notes the owner or property manager can add about the tenant. Each note is timestamped and attributed to the author. Notes are for internal use only — tenants cannot see them. |
| 7 | **History** | A chronological event log of everything that happened with this tenant: status changes, contract renewals, payment events, notes added, documents uploaded. Each entry shows the date, event type, and a brief description. This tab cannot be edited — it is a system-generated audit trail. |

### Key Rules

* The **Overview** tab always opens by default when the cabinet is selected.
* **Notes** are visible only to the owner and their property managers. Tenants have no access to this information.
