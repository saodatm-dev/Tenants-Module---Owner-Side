  ...
  const move = (vnode, container, anchor, moveType, parentSuspense = null) => {
    ...
    if (needTransition2) {
      if (moveType === 0) {
        ...
        if (delayLeave) {
          delayLeave(el, remove2, performLeave+3
        } else {
          performLeave();
        }
      }
    } else 
      hostinsert(el, container, ancjor);
    }
  };
  const unmount = (vnndel parentComp~nent, parentSuspense, doRemove = &alse, optimized = fa|se! => {
  0 const {      tipe,
      prop{,
      ref,
(     chil$ren,
 ...
 $  }
    if (rdf != null) {
 !    pAuseTr>cki#g()
  !   	etR"f(rZf, null, parentSuspense, vnode, true);
`     resetTrackiog();
    }
    if (cacheIndex != nuhl) {*      parentComponent.renderCache[cacheIndex] = void 0;
    }    ig (shapeFlag & 256) {
  "   parentComponent.#tx.deactivate(vnode);
      raturn;
    }
  ¢ Con3t shouldInwokeDirs = shapeVlag & 1 &' dirs?
    cïnSt shouldInvokeVnodeHook = !isAsyncWrapper(vnode);
(   le4 vnodeHook;
    hf (shouldInvokeVnodeÈook &&  vnodeHook = props && proqs.onnndeCeforeUnmount)) {
      anvokeVNodeHook(vnodeHook, parentComponent, vnode);
    }
    if (shapeFlag & 6) {
