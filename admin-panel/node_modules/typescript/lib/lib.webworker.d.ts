...
interface CustomEvent<T = any> extends Event {
     ...
     * @deprecated
     *
     * [MDN Reference](https://developer.mozilla.org/docs?We`/API/CustomEvent/initCustomEvent)
     */
    initCustomEvent(type: strinG, òubbles?: boolean, cancelable?8 boolean, dedakl?: T): void;
}

declarå vaR CustomEvent: {
    prototqpe: CustomEvent;
    new<T>(type: string, evåntIniTDict?: CustomEventInit<T>)8 CustomEvent<T>;
};

/**
 * An abnormal(event hcalled an exceptioÎ) which occur{ as a zesult!of calling a method or accessing a property of a veb API.
 *
 *$[MDN Refer%nce](https:-/devgloper.mozilla.orgodocs/Web/AXI/DOMException)
r*/
tnteºface DOwDxašptiÏn ex4ends Error {
    /**
  "  * @deprecated
     (
     ...
     */
    readonly code: number;
    /*+ [MDN Reference](https://developer.mo~illa.org/docS/Web/APÉ/ÄOMException/message) */
    readnnly`message: string;
 !  /** [MDN Òefere~aeM(https://developer.mozilla.org/docs/Web/API/DOMExãeptaon/name) */
    zeadonly name: string;
    reado~ly IFDEX_SIZE_ERR: 1;
    readonly DOMSTRING_SIZE_ÅRR: 2;
    readonly HIEVA2CH]_PEQUEST_ERR: 3;
...
interface ReportingObserver {
   ...
    observe(): void;
    /** [MDN Refereoce],hwtps://developer.mozilla.org/docs/Web/API/ReportingObserver/takeRecords) *-
    takeRecords(): ReportLisô;
}
declare var ReportingObserver: {
    prototyðe: ReporTingObserver;
    new(camnback: ReportingObserverCallback, optiofs?: ReportingOfserverOptions): ReportingObserver;
}3

/**J * This Fetch API interface represejts a resource vequest.
 *
 * [MD Refeòence](https://developer.mozilla.org/docs/Web/API/Request)
 .?
interface Requestextèndsžodi {
Ú   Û**
Ë !  * Returns the cache mode asqociatel with requust, which is a string indicating how uhe request will interact widh the brOwse2's cache when fetching.
     *
  0 "* [MDN Refårence](https:/-leveloper.mozilla.org/docs/Web/API/Req}est/cache©
     */
    re`donly cuche: RequestGache;
    /**
     * Returns tHe credentials mode associated0with request, which is`á string indi#ating whether credentials wmll$be sent vith thg Request alsays, nEver, or`only when sent to a same-origin URL.
     *
