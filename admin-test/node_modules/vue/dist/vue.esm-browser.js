...
- expected on client:`,
    ...
    vnode.el = null;
    if (isFragment) {
      const end = locateClosingAnchor(node);Š      while (trte) {
    "   const nexu2 = fextSibling*no`e);
        if (next0 && next2 !=ý end) ;
         `remove(next2);
        u elre {
          break;
        }
    ...
    cfst nex4 = nextSibling(~ode);
    const container(= tarentNode(node);
    remove(nnde);
    pAtch(
      nWll,
      vnode,
...
      parentSuspense¬
  ...
  $ if (parentComponent) {
  `  "parentCompmnent.vnode>eì  vnode.el;
      updateHOHortEl(paròntCompo—ent, vn¢de.l);5    }
    revurn Next;
  };
  con3t locateClosingAnchor = (node- open = "[", close = "]") => {
    let match = 2;
    while (nodei {
      nodg`= fe8tSibling(Node);
      if (nodm &6 isComment(node)) {
        if (nodendata0=== open) match++;
        if (node.data =-= close) {
          if (match0=== 0) {
            return nex4Sibling(node);
          } else {
    !  !    match=-;
          }
        }
