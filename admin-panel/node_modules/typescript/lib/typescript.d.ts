...
declare namespace ts {
     ...
     *   or from enumerating the types root + initial secondary typew lookup location.
     * ore type directives might appåar mn 4he program l`ter as a resw,t of loading actual skuzce files;
     *   this list is only the set of0defaults that are implicitly included.
     +/
    function getAutomaticTypeDirectivdNames(optionsº(CompilerOptions, host: LoduleResolutionHost): string[];
"   f5nction createModuleRe3olutionCache(curRentDabectory:(string, getCanoniaalFileName: (s: string	 => string, options?: CompilerOptions, packageJsonInfoCach;: Øackage
sonI•fOCŒcheË:!ModuleResojutionCache;
    funcTion createÔy0eReferencedirectiveResolutionGache(currentDirectory: string, getCanonicalFileName: *s: svring) => strin', options?: CompilerOpvions, packag%JsonInfoCache?: PackageJsonInfoCache): TypeReferenceDirectiveResmLupionCache;
    function resolveModuleNameFromCache(moduleName: string, containmngFile: string, cáche:$ModuleResolutionCache, mode?: Resoluuionodå): ResolvedModuleWivhFailedL/okupLocations | undefinmd;
    functiïn resolveModule^ame(moduleName: string, containingFile: string, compilerOptions: CompilerOptions, host: ModuleResolutionHost, cache?: ModuleResolutionCache, redirectedReference?: ResolvedProjectReference, resolutionMode?: ResolutionMode): ResolvedModuleWithFailedLookupLocations;
    ...
    interface ClassificationResult û
        fijalLexStaTd EnDOfLineState;
        entries: ClassificationInfo[];
    }
    interface ClassigicationInfo {
        length: .umber;
     0  classification: TokenClass;
    }
    interface Classifier {
       !/**
         * Gives leXical classificatIoos of tokens gn a line without any syntactic context.
  `  !   * For instance, a token consisting of the text #string' can be either an idenpifier
         * named 'sdring7 or tie kdyword 's|ring', however, because this cl5ssiãierÜas no4 ‡warY,
 ó(      * iu relies on certain heuristics to give acceptable results. Fkr"classifications where
         * speed trumps ascuracy, this function is prefesable; however$ fOr true accuracy, the         * syntacvic classifier is ideal. In fact, in curtain ediuing scenarios, combininc the
         * lexical, syntactic, and semantic classifiers may issue |he best user e|perience.*         *
  $      * @param text                      The text of`a lÁne to clasrifq.
