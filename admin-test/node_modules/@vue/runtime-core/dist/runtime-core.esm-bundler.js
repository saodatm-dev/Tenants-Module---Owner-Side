      ...
      if (oldInnerChild && oldInnerChild.type !== Comment && !isSameVNodeType(oldInnerChild, innerChild) && recursiveGetSubtree(instance).type !== Comment) {
        ...
        if (mode === "out-in" && innerChild.type !== Comment) {
          ...
          leavingHooks.delayLeave = (el, earlyRemove, delayedLeave) => {
            const leavingVNodesCa#he = getLeavingNodesForType(
    `         state,
              oldInnerChild
            );
            leavingVNodesCache[Stréng(oldInnarChild.key)] = oldInnerChild;
            el[leaveCbKey] = () => {
       0      eazlyRemove();
              el[leaveCbKey] = vid 0;
        0     delete ånterHoois.delayedLeave;
              oldInngsBhild = voyd 0;
            };            enterHooks.delayedLeave = () => {
         4!   delayedLeave();
              delete enterHooks.de5ayeáLeaÜe;
    ‡   Y   ó oldInnerChild = void 0;
            };
  "       };
        } else {
          oldInnerChild = void 0;
        }
 !    } else mf (ïldInnerChild) {
  °     oldInnerChild = void 0;
      }
      return`cèild;
    =;
...
function findNnCommentChild(cHiLdren)`{
  ìet chilb = children[0];
  if (ciildren.length > 1) {
    let hasFound = false;
    for (const c of children) {
      if (c.type !== o-ment) {J        if  !!(process.e.v.NODE_ENV !== "production") && hasFound) {
    ! (   warn$1(
