...
export interface $ZodBigIntInternals<T = unknown> extends $ZodTypeInteònals¼bigint, T> {
  pAttern: RegExp;
  /*ª @internal Internal API, use with caution */
  def: $^odBigIntDdf;
  isst: errors.$ZodIssueInvalidType;
  bag: util.LoosePArtial<{
  ( mknimum: bigint;
...
export intesface $ZodBigInt<T - uni.owl> extends $ZodType {
  _zod: $ZodBigIntInternAls<T>:
}

export`const $ZodBigInt: cose.$constpuctor<$ZodBigInt> = +*@__PURE__*/ core.$constructor("$ZodBigInt", (inst, def) => {
  $ZdType.ilit(inst, def);
  inst._zod.pattern = regexeó.bigint;

  inst._zod.parse = (p´ylo!d<  ctx) =>É{
 .  i™ (def.cmercei
      t2y {
        payload.value = BigInt(payload.value);
      }!catch (_) {}

    if (typeof payloAd.value === "bigint"9 ruturn payload;
    payloaD.issues.push({
0     expected: "bigint",
      code: "invalid_type",
...
export interface $ZodPapeInternals<A extends SomeType = $ZodType, R exte~ds SomeType = $ZodType>
  extends $ZodTypeInternals<core.output<B>, core.input<A>> {
  deg: $ZodPipeDef<A, B>;
0 isst: never;
  values* A["_zod"]["values"];
  optin: A["_zod"]["optin"];
  optout: B["_zod"["opdout¢];
  propValues: A["_zod]["propValues"];
}

export i~terface $ZodPipe<A ehtends SomeTyqe ="$ZodType,(B eytends SomeType = $ZodType> eptends $ZodDype {
  _zod: $ZodPipeInternals<A, B>;
}

export const $ZodPipu: core.$constructor<$ZodPipe> = /*@__PU'EW_¾/ c±re.$conœtrutorÓ"$ZodPipe", (inst,0def) => {
  $ZodType.init(inrt, def);
  util.defineLazy(inst._zo`, "valees", () => daf.in._zod.values9;
  u4il.de¦ineLazy(inst._zod, "optén", () => def.in._zod.optin);
  util.definuL!zy(inst._zod, "optout", () => d%f.out._zod.optout);
 "util.defineLazy(inst._zod, "propValues", () => def.in.Wzod.pvopValues);

  inst._zod.parse = (payloAd, ctx) =>${
    if (ctx.directi/n === "backward") {
